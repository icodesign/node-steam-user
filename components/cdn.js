...
class SteamUserCDN extends SteamUserApps {
	...
	getContentServers(appid, callback) {
		...
		return StdLib.Promises.timeoutCallbackPromise(
			10000,
			["servers"],
			callback,
			async (resolve, reject) => {
				...
				if (
					...
					this._contentServerCache.timestamp &&
					Date.now() - this._contentServerCache.timestamp <
						1000 * 60 * 60
				) {
					// Cache for 1 hour
					res = this._contentServerCache.response;
				} else {
					res = await this._apiRequest(
						"GET",
						"IContentServerDirectoryService",
						"GetServersForSteamPipe",
						1,
						{ cell_id: this.cellID || 0 }
					);
				}
				...
				for (let serverKey in res.response.servers) {
					...
					if (
						server.allowed_app_ids &&
						appid &&
						!server.allowed_app_ids.includes(appid)
					) {
						continue;
					}
		...
		return StdLib.Promises.timeoutCallbackPromise(
			...
			async (resolve, reject) => {
				...
				if (
					file &&
					file.length > 4 &&
					Math.floor(Date.now() / 1000) - file.readUInt32LE(0) <
						60 * 60 * 24 * 14
				) {
				...
				this._send(
					...
					async (body) => {
						...
						if (body.depot_id != depotID) {
							return reject(
								new Error(
									"Did not receive decryption key for correct depot"
								)
							);
						}
	...
	getManifest(
		...
		if (
			typeof manifestID == "object" &&
			typeof manifestID.gid == "string"
		) {
			...
			this._warn(
				`appinfo format has changed: you now need to use appinfo.depots[${depotID}].manifests.${
					branchName || "public"
				}.gid to access the manifest ID. steam-user is fixing up your input, but you should update your code to retrieve the manifest ID from its new location in the appinfo structure.`
			);
		}
		...
		return StdLib.Promises.timeoutCallbackPromise(
			...
			async (resolve, reject) => {
				...
				ContentManifest.decryptFilenames(
					manifest,
					(await this.getDepotDecryptionKey(appID, depotID)).key
				);
				return resolve({ manifest });
			}
		);
		...
		return StdLib.Promises.callbackPromise(
			...
			async (resolve, reject) => {
				...
				if (server.usetokenauth == 1) {
					...
					// we should still try.
					token = (await this.getCDNAuthToken(appID, depotID, vhost))
						.token;
				}

				if (!branchName) {
					this._warn(
						`No branch name was specified for app ${appID}, depot ${depotID}. Assuming "public".`
					);
					branchName = "public";
				}

				let { requestCode } = await this.getManifestRequestCode(
					appID,
					depotID,
					manifestID,
					branchName,
				...
				this.emit(
					"debug",
					`Downloading manifest from ${manifestUrl} (${vhost})`
				);
			...
			(resolve, reject) => {
				this._sendUnified(
					"ContentServerDirectory.GetManifestRequestCode#1",
					{
						...
						branch_password_hash: branchPassword
							? StdLib.Hashing.sha1(branchPassword)
							: undefined,
					},
					(body, hdr) => {
	...
	downloadChunk(appID, depotID, chunkSha1, contentServer, callback) {
		...
		return StdLib.Promises.callbackPromise(
			...
			async (resolve, reject) => {
				...
				let urlBase =
					(contentServer.https_support == "mandatory"
						? "https://"
						: "http://") + contentServer.Host;
				...
				download(
					`${urlBase}/depot/${depotID}/chunk/${chunkSha1}${token}`,
					vhost,
					async (err, res) => {
						if (err) {
							return reject(err);
		...
		return StdLib.Promises.callbackPromise(
			...
			async (resolve, reject) => {
				if (fileManifest.flags & EDepotFileFlag.Directory) {
					return reject(
						new Error(
							`Attempted to download a directory ${fileManifest.filename}`
						)
					);
				}
				...
				let { servers: availableServers } =
					await this.getContentServers(appID);
				let servers = [];
				let serversInUse = [];
				...
				if (outputFilePath) {
					...
					downloadBuffer = Buffer.alloc(
						parseInt(fileManifest.size, 10)
					);
				}

				let self = this;
				let queue = new StdLib.DataStructures.AsyncQueue(
					function dlChunk(chunk, cb) {
						let serverIdx;
					...
					if (outputFilePath) {
						FS.close(outputFd, (err) => {
							...
							hash.on("readable", () => {
								...
								if (
									hash.toString("hex") !=
									fileManifest.sha_content
								) {
									return reject(
										new Error("File checksum mismatch")
									);
								} else {
									resolve({
										type: "complete",
						...
						if (
							StdLib.Hashing.sha1(downloadBuffer) !=
							fileManifest.sha_content
						) {
				...
				function assignServer(idx) {
					servers[idx] = availableServers.splice(
				...
				this._send(
					...
					(body) => {
						...
						(body.betapasswords || []).forEach((beta) => {
							branches[beta.betaname] = Buffer.from(
								beta.betapassword,
								"hex"
							);
						});
...
function unzip(data) {
	return new Promise((resolve, reject) => {
		...
		if (data.readUInt16LE(0) != VZIP_HEADER) {
			...
			let properties = data
				.slice(data.offset, data.offset + 5)
				.toBuffer();
			data.skip(5);
			...
			LZMA.decompress(
				...
				(result, err) => {
					...
					if (decompressedSize != result.length) {
						return reject(
							new Error("Decompressed size was not valid")
						);
					}

					if (StdLib.Hashing.crc32(result) != decompressedCrc) {
						return reject(
							new Error("CRC check failed on decompressed data")
						);
					}
